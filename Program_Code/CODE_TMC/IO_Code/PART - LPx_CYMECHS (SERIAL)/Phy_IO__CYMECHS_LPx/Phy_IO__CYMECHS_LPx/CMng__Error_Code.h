...
public:
	...
	void Clear__Error_Code()
	{
		...
		sList__ERR_TITLE.RemoveAll();
		sList__ERR_MSG.RemoveAll();

		LeaveCriticalSection(&mLOCK);
	}
	void Load__Error_Code(const CString& err_code,const CString& err_title,const CString& err_msg)
	{
		EnterCriticalSection(&mLOCK);
	...
	int  Get__Error_Code(CString& err_code,CString& err_title,CString& err_msg)
	{
		EnterCriticalSection(&mLOCK);
