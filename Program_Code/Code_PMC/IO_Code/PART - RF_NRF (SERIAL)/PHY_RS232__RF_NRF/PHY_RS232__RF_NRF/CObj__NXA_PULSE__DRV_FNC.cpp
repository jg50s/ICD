	...
	|| (actove__get_exec))
	{
		...
		if(r_len > 8)
		{
			...
			if(actove__param_a1)
			{
				...
				data_freq._byte[2] = r_data[0 + r_offset];
				data_freq._byte[1] = r_data[1 + r_offset];
				data_freq._byte[0] = r_data[2 + r_offset];
			
				_DATA__2BYTE_INT data_duty;
				data_duty._byte[1] = r_data[3 + r_offset];
				data_duty._byte[0] = r_data[4 + r_offset];
			...
			else if(actove__param_a2)
			{
				...
				data__on_time._byte[2] = r_data[0 + r_offset];
				data__on_time._byte[1] = r_data[1 + r_offset];
				data__on_time._byte[0] = r_data[2 + r_offset];

				_DATA__2BYTE_INT data__off_time;
				data__off_time._byte[2] = r_data[3 + r_offset];
				data__off_time._byte[1] = r_data[4 + r_offset];
				data__off_time._byte[0] = r_data[5 + r_offset];

				ch_data.Format("%.2f", dUNIT__uSEQ*data__on_time._hexa);
				sCH__INFO_PARAM_A_ON_TIME->Set__DATA(ch_data);

				ch_data.Format("%.2f", dUNIT__uSEQ*data__off_time._hexa);
