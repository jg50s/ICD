...
public:
	...
	// ...
	void Load__ALM_OFFLINE(const int gas_id,const CString& alm_msg);
	void Load__ALM_CTRL_TIMEOUT(const int gas_id,const CString& alm_msg);
...
public:
	...
	// ...
	int Fnc__FLOW_SETPOINT_SET(const CString& fnc_name,const double set_data,const double min,const double max,const CString& cmmd1,const CString& cmmd2,const CString& cmmd3);
	int Fnc__FLOW_SETPOINT_GET(const int cmmd,const int gas_num,const double max,double& read_data);

	int Fnc__VALVE_CONTROL_SET(const int ctrl_mode,const int cmmd,const int gas_id);
};
