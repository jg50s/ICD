...
::__Read__ANALOG(const CString& fnc_name, const CDS_IO__CHANNEL_INFO& io_info, double& read_data)
{
	...
	// ...
	int nCmd = atoi(fnc_name);
	int nGasNum = atoi(io_info.sCOMMAND1);
	...
	switch(nCmd)
	{
		...
		case 101:
		{
			...
			nSizeOfRecv = 12;
			szSendData[0] = m_nBoardNumber +  (nGasNum -1);
			szSendData[1] = STX;
		...
		case 104:
		{
			...
			if(atof(m_szFirm_Revision[nGasNum-1]) >= 4.0)
			{
				nSizeOfRecv = 11;
			}
			else
	...
	if(nCmd_Ret < 0)
	{
		// ...
		{
			...
			log_msg.Format("<-- Fail Read Analog:Fnc_Name:[%s], GAS_NUM:[%d], [%s] Value:%.2f ...",
						   fnc_name, nGasNum, str_cmd_name, read_data);
			log_msg += "\n";

			log_bff.Format("Read__ANALOG_EX [%s] GAS_NUM [%d] <-- OUT, ret:%d", 
						   fnc_name, nGasNum, nCmd_Ret);
			log_msg += log_bff;
			log_msg += "\n";
	...
	switch(nCmd)
	{
		...
		case 104:		
		{
			...
			if(recv_macid == (m_nBoardNumber + nGasNum-1))
			{
				szRcvData.Format("%d", (m_nBoardNumber + nGasNum-1));
			}
			else
			{
				szRcvData = "99";		
...
::__Read__DIGITAL(const CString& fnc_name,const CDS_IO__CHANNEL_INFO& io_info, CString& read_data,int& item_index)
{
	...
	switch(nCmd)
	{
		case 201:
		{
			if(m_nSubOnline[nGasNum - 1] == OFFLINE)
			{	
				read_data = "OFFLINE";
			}
			else
...
::__Read__STRING(const CString& fnc_name, const CDS_IO__CHANNEL_INFO& io_info, CString& read_data)
{
	...
	switch(nCmd)
	{
		case 301:
		{
			read_data.Format("%s", m_szFirm_Revision[nGasNum-1]);
		}
		break;
	}
		...
		case 150:	// 1. [Setpoint] Make The Packet !!
		{
			while(1)
			{
				...
				if(dEXT_CH__CFG_MFC_485_SETPOINT_READ_CONFIRM_USE->Check__DATA(STR__ENABLE) > 0)
				{
					...
					if(fnc_flag > 0)
					{
						fnc_flag = Fnc__FLOW_SETPOINT_SET(fnc_name, set_data, min_value, max_value, cmmd1,cmmd2,cmmd3);
					}

					if(fnc_flag > 0)
					{
						fnc_flag = Fnc__FLOW_SETPOINT_GET(nCmd, gas_id, max_value, read_data);
					}
					...
					if(fnc_flag > 0)
					{
						if((read_data >= min_set) && (read_data <= max_set))
						{
							if(set_data < 0.001)
							{
								Fnc__VALVE_CONTROL_SET(VLV_CTRL_MODE__CLOSE, nCmd, gas_id);
							}
							else
							{
								Fnc__VALVE_CONTROL_SET(VLV_CTRL_MODE__CONTROL, nCmd, gas_id);
							}
							return 1;
						}
					}
...
int CObj__UNIT_TYPE
::Fnc__FLOW_SETPOINT_SET(const CString& fnc_name, 
						 ...
						 const CString& cmmd1,const CString& cmmd2,const CString& cmmd3)
{
	...
	// ...
	{
		...
		log_bff.Format("Write__ANALOG_EX [%s] GAS_NUM [%d] --> IN", fnc_name, nGasNum);
		log_msg += log_bff;
		log_msg += "\n";

		log_bff.Format("** Set => [%f] sccm, Fnc_ID:[%d], GAS_NUM[%d] Start..", set_data , nCmd, nGasNum);
		log_msg += log_bff;
		log_msg += "\n";
	}
	...
	// ...
	{
		log_bff.Format("--> Start Write Analog:Gas_Num[%d] Action[%s] ...",nGasNum, str_cmd_name);
		log_msg += log_bff;
		log_msg += "\n";
	...
	// ...
	{
		...
		if(nCmd_Ret < 0)
		{
			log_bff.Format("Set => [%f] sccm, Fnc_ID:[%d], GAS_NUM[%d] Failed..ret:-1", 
				           set_data, nCmd, nGasNum);
			log_msg += log_bff;
			log_msg += "\n";

			log_bff.Format("Error :: Write__ANALOG_EX [%s] GAS_NUM [%d] <-- OUT, ret:%d", 
				           fnc_name, nGasNum, nCmd_Ret);
			log_msg += log_bff;
			log_msg += "\n";
		...
		log_bff.Format("Write__ANALOG_EX [%s] GAS_NUM [%d] <-- OUT", fnc_name, nGasNum);
		log_msg += log_bff;
		log_msg += "\n";
...
int CObj__UNIT_TYPE
::Fnc__FLOW_SETPOINT_GET(const int cmmd, const int gas_num, const double max, double& read_data)
{
	// ...
	UCHAR szSendData[MAX_DATA_LENGTH];
	UCHAR szReadData[MAX_DATA_LENGTH];
	...
	// ...
	{
		...
		nSizeOfRecv = 12;
		szSendData[0] = m_nBoardNumber + (gas_num -1);
		szSendData[1] = STX;
		szSendData[2] = 0x80;
		szSendData[3] = 0x03;
		szSendData[4] = 0x69;
	...
	// ...
	{
		...
		log_bff.Format("Read__ANALOG_EX : GAS_NUM [%1d] --> IN", gas_num);
		log_msg += log_bff;
		log_msg += "\n";

		log_bff.Format("--> Start Read Analog:Gas_Num[%d] Action[%s] ...", 
			           gas_num,str_cmd_name);
		log_msg += log_bff;
		log_msg += "\n";
	...
	if(nCmd_Ret < 0)
	{
		log_bff.Format("<-- Fail Read Analog : GAS_NUM:[%d], [%s] Value:%.2f ...", 
			           gas_num, str_cmd_name, read_data);
		log_msg += log_bff;
		log_msg += "\n";
						 ...
						 const int gas_id)
{
	...
	if(ctrl_mode == VLV_CTRL_MODE__CLOSE)
	{
		fnc_name = "Fnc__VLV_CLOSE_SET()";
	}
	else if(ctrl_mode == VLV_CTRL_MODE__CONTROL)
	{
		fnc_name = "Fnc__VLV_CONTROL_SET()";
	}
	else
	{
		return -1;
	...
	// ...
	{
		...
		log_bff.Format("%s <-- GAS_NUM [%d] : Started", fnc_name,gas_id);
		log_msg += log_bff;
		log_msg += "\n";
	...
	if(ctrl_mode == VLV_CTRL_MODE__CONTROL)
	{
		...
		nSizeOfRecv = 2;
		szSendData[0] = m_nBoardNumber + (gas_id -1);
		szSendData[1] = STX;
		szSendData[2] = 0x81;
		szSendData[3] = 0x04;
		szSendData[4] = 0x6a;
	...
	// ...
	{
		str__log.Format("--> Start Write Digital: Gas_Num[%d] ...", gas_id);
	...
	// ...
	{
		str__log.Format("--> SendCommand : Ret(%d) ...", gas_id); 
	...
	if(nCmd_Ret < 0)
	{
		...
		log_bff.Format("Error :: Set Error => GAS_NUM[%d] ... (ret : %1d)", gas_id,nCmd_Ret);	
		log_msg += log_bff;
		log_msg += "\n";
	...
	// ...
	{
		...
		log_bff.Format("Set Complete => GAS_NUM[%d] ... (ret : %1d)", gas_id,nCmd_Ret);
		log_msg += log_bff;
		log_msg += "\n";
...
::__Write__DIGITAL(const CString& fnc_name,const CDS_IO__CHANNEL_INFO& io_info, const CString& set_data,const int item_index)
{
	...
	// ...
	{
		...
		log_bff.Format("Write__DIGITAL_EX [%s] GAS_NUM [%d] --> IN", fnc_name,nGasNum);
		log_msg += log_bff;
		log_msg += "\n";

		log_bff.Format("** Set => [%s], Fnc_ID:[%d], GAS_NUM[%d] Start..", set_data,nCmd,nGasNum);
		log_msg += log_bff;
		log_msg += "\n";
	...
	// ...
	{
		str__log.Format("--> Start Write Digital:Gas_Num[%d] Action[%s] ...",
						nGasNum, str_cmd_name);
		Fnc__DRV_LOG(str__log);
	}
	...
	if(nCmd_Ret < 0)
	{
		...
		log_bff.Format("Set => [%s], Fnc_ID:[%d], GAS_NUM[%d] Failed..ret:-1", set_data, nCmd, nGasNum);
		log_msg += log_bff;
		log_msg += "\n";

		log_bff.Format("Write__DIGITAL_EX [%s] GAS_NUM [%d] <-- OUT, ret:%d ", fnc_name, nGasNum, nCmd_Ret);
		log_msg += log_bff;
		log_msg += "\n";
	...
	switch(nCmd)
	{
		case 251:
		{
			m_szFirm_Revision[nGasNum-1].Format("%c%c%c%c",
				                                szReadData[8], 
												szReadData[9], 
												szReadData[10], 
												szReadData[11]);
			   ...
			   const int nCmd)
{
	...
	{
		if(i == nPaddingSize + nLength)
		{
			szCommand[i] = uchCheckSum;
		}
		else
		{
			szCommand[i] = PADDING;
	...
	if((nCounter >= 2) || (check_flag < 0))
	{
		...
		if(dEXT_CH__CFG_MFC_485_COMM_OFFLINE_ALARM_USE->Check__DATA(STR__ENABLE) > 0)
		{
			...
			mDB__ALM_CTRL.Load__ALM_OFFLINE(nGasNum,alm_msg);
		}
		m_nSubOnline[nGasNum-1] = OFFLINE;

		// ...
		{
			CString log_msg;

			log_msg.Format("Error :: [%d] OFFLINE", nGasNum);	
			Fnc__DRV_LOG(log_msg);
		}
		return -1;
	}
