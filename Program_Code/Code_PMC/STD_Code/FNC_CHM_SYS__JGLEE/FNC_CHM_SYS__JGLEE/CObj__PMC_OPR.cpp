...
int CObj__PMC_OPR::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// PARA ...
	{
		str_name = "PARA.MANUAL_MOVE.SLOT_CLOSE.SKIP";
		STD__ADD_STRING(str_name);
		LINK__VAR_STRING_CTRL(sCH__PARA_MANUAL_MOVE_SLOT_CLOSE_SKIP, str_name);

		str_name = "PARA.MANUAL_MOVE.LIFT_PIN.SKIP";
		STD__ADD_STRING(str_name);
		LINK__VAR_STRING_CTRL(sCH__PARA_MANUAL_MOVE_LIFT_PIN_SKIP, str_name);
	}
...
int CObj__PMC_OPR::__CALL__CONTROL_MODE(mode,p_debug,p_variable,p_alarm)
{
	...
	if((flag < 0)||(p_variable->Check__CTRL_ABORT() > 0))
	{
		...
		// ...
		{
			sCH__PARA_MANUAL_MOVE_SLOT_CLOSE_SKIP->Set__DATA(STR__END);
			sCH__PARA_MANUAL_MOVE_LIFT_PIN_SKIP->Set__DATA(STR__END);
		}
	}
	else
	{
