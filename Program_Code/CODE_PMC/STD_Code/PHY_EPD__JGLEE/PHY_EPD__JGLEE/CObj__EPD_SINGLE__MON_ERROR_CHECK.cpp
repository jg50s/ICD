...
::Mon__ERROR_CHECK(CII_OBJECT__VARIABLE *p_variable, CII_OBJECT__ALARM *p_alarm)
{
	...
	// Range.Error ...
	{
		...
		// Idle ...
		{
			...
			if(bActive__EPD_SIGNAL_MAIN)
				error__idle.Link__REF_IN_CHANNEL(aEXT_CH__EPD_SIGNAL_MAIN);
	...
	while(1)
	{
		...
		if(loop_count == 1)
		{
			...
			if(bActive__EPD_SIGNAL_MAIN)		aEXT_CH__EPD_SIGNAL_MAIN->Change__MIN_MAX_DEC(cfg_min, cfg_max, cfg_dec);
