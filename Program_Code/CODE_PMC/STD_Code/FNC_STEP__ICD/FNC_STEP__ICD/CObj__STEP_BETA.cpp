...
//-------------------------------------------------------------------------
CObj__STEP_BETA::CObj__STEP_BETA()
{
...
//-------------------------------------------------------------------------
int CObj__STEP_BETA::__DEFINE__CONTROL_MODE(obj,l_mode)
{
	sObject_Name = obj;
...
int CObj__STEP_BETA::__DEFINE__VERSION_HISTORY(version)
{
...
int CObj__STEP_BETA::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// RCP ...
	{
		...
		// FRC ...
		{
			var_name = "RCP.FRC.CENTER.PERCENT";
			STD__ADD_ANALOG_WITH_OPTION(var_name, "%", 1, 0.0, 100.0, -1, "E", "");
			LINK__VAR_ANALOG_CTRL(aCH__RCP_FRC_CENTER_PERCENT, var_name);
		}
...
int CObj__STEP_BETA::__DEFINE__ALARM(p_alarm)
{
	DECLARE__ALARM;
...
int CObj__STEP_BETA::__Define__USER_FUNCTION(CII_DEFINE__FUNCTION *p_fnc_ctrl)
{
...
//-------------------------------------------------------------------------
int CObj__STEP_BETA::__INITIALIZE__OBJECT(p_variable,p_ext_obj_create)
{
	CString def_name;
	CString def_data;
	CString ch_name;
	...
	// OBJ.FRC ...
	{
		...
		def_check = x_utility.Check__Link(obj_name);
		bActive__OBJ_CTRL__FRC = def_check;

		if(def_check)
		{
			pOBJ_CTRL__FRC = p_ext_obj_create->Create__OBJECT_CTRL(obj_name);

			//
			var_name = "PARA.SET_RATIO.MASTER";
			LINK__EXT_VAR_ANALOG_CTRL(aEXT_CH__FRC_OBJ__CENTER_Wt_SETPOINT, obj_name,var_name);
		}
	}
...
//-------------------------------------------------------------------------
int CObj__STEP_BETA::__CALL__CONTROL_MODE(mode,p_debug,p_variable,p_alarm)
{
	int flag = -1;
...
int CObj__STEP_BETA::__CALL__MONITORING(id,p_variable,p_alarm)
{
