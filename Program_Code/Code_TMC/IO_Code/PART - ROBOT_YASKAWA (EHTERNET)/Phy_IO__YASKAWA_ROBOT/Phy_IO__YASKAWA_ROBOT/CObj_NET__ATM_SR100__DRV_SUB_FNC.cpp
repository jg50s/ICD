		...
		if(mMng__RSP_MSG.Get__RSP_MSG(rsp_msg) > 0)
		{
			...
			if(rsp_msg.GetLength() > 0)
			{
				...
				if(ch_rsp == '!')
				{
					...
					if(bActive__Recv_Cnt_Single)
					{
						if(cnt_vent >= 1)		return _DRV_RSP__COMPLETE;		
					}
					else
					{
						if(cnt_vent >= 3)		return _DRV_RSP__COMPLETE;		
					}
				}
			}
		}
...
int  CObj_NET__ATM_SR100
::_Update__ROBOT_STATUS(const CString& r_data)
{
	unsigned char i_sts1 = 0x0;
	unsigned char i_sts2 = 0x0;
	unsigned char i_sts3 = 0x0;
	unsigned char i_sts4 = 0x0;

	int i_len = r_data.GetLength();
	if(i_len > 0)		i_sts1 = _Get__HEXA_FROM_ASCII(r_data.GetAt(0));
	if(i_len > 1)		i_sts2 = _Get__HEXA_FROM_ASCII(r_data.GetAt(1));
	if(i_len > 2)		i_sts3 = _Get__HEXA_FROM_ASCII(r_data.GetAt(2));
	if(i_len > 3)		i_sts4 = _Get__HEXA_FROM_ASCII(r_data.GetAt(3));

	// Status1 ...
	{
		unsigned char x_sts = i_sts1;

		if(x_sts & 0x01)		sCH__MON_RB_STS__WAFER_SNS_ON_BLADE_1->Set__DATA(STR__ON);
		else					sCH__MON_RB_STS__WAFER_SNS_ON_BLADE_1->Set__DATA(STR__OFF);

		if(x_sts & 0x02)		sCH__MON_RB_STS__WAFER_SNS_ON_BLADE_2->Set__DATA(STR__ON);
		else					sCH__MON_RB_STS__WAFER_SNS_ON_BLADE_2->Set__DATA(STR__OFF);

		if(x_sts & 0x04)		sCH__MON_RB_STS__WAFER_GRIP_ON_BLADE_1->Set__DATA(STR__OFF);
		else					sCH__MON_RB_STS__WAFER_GRIP_ON_BLADE_1->Set__DATA(STR__ON);

		if(x_sts & 0x08)		sCH__MON_RB_STS__WAFER_GRIP_ON_BLADE_2->Set__DATA(STR__OFF);
		else					sCH__MON_RB_STS__WAFER_GRIP_ON_BLADE_2->Set__DATA(STR__ON);
	}
	// Status2 ...
	{
		unsigned char x_sts = i_sts2;

		unsigned char bit_check = 0x01;
		...
		for(i=0; i<i_limit; i++)
		{
			if(i == 0)		bit_check = 0x01;
			else			bit_check *= 0x02;
	
			if(x_sts & bit_check)		sCH__MON_RB_STS__INTERLOCK_SIGNAL_X[i]->Set__DATA(STR__OK);
			else						sCH__MON_RB_STS__INTERLOCK_SIGNAL_X[i]->Set__DATA(STR__ERROR);
		}
	}
	// Status3 ...
	{
		unsigned char x_sts = i_sts3;
		...
		for(i=0; i<i_limit; i++)
		{
			...
			if(x_sts & bit_check)		sCH__MON_RB_STS__INTERLOCK_SIGNAL_X[i+4]->Set__DATA(STR__OK);
			else						sCH__MON_RB_STS__INTERLOCK_SIGNAL_X[i+4]->Set__DATA(STR__ERROR);
		}
	}
