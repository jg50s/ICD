...
::Get__LLx_Of_Route_Info(const int ll_type)
{
	...
	for(i=0; i<i_limit; i++)
	{
		CDB__Category_Info *p_info = (CDB__Category_Info *) pList_Category[i];

		if(p_info->iImg_ID != check_img)		continue;
		...
		for(k=0; k<k_limit; k++)
		{
			CDB__Module_Info *p_md = p_info->Get__Module_Info(k);

			CString md_id;
			CString md_mode = "ALL";
			CString md_slot = "11";
			CString md_cooling_sec = "0";
			...
			for(p=0; p<p_limit; p++)
			{
				...
				else if(p == 2)		md_cooling_sec = para_data;
			}
	...
	for(i=0; i<i_limit; i++)
	{
		...
		for(k=0; k<k_limit; k++)
		{
			...
			for(p=0; p<p_limit; p++)
			{
				...
				else if(p == 3)		md_n2_purge = para_data;
			}

			str_data.Format("#STx#,%s,%s,%s,%s,%s;\n",
							md_id, 
							md_mode, 
							md_slot, 
							md_sec,
...
::Get__PROCESSx_Of_Route_Info()
{
	...
	for(i=0; i<i_limit; i++)
	{
		...
		for(k=0; k<k_limit; k++)
		{
			...
			int pm_id = MACRO__Get__PMC_ID(p_md->sTitle);
